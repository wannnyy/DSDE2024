...
fig_elbow = px.line(x=list(range(1,len(inertias)+1)), y = inertias)
...
feat_x, feat_y = feature_names[2], feature_names[3]  # petal length & petal width
with col1:
    ...
    fig_col1 = px.scatter(
        ...
        color='Cluster',
        color_discrete_map=colors,
        title=f'KMeans (k={clusters}) on {feat_x} vs {feat_y}',
...
with col2:
    ...
    fig_col2 = px.scatter(
        ...
        color='Species',
        color_discrete_map=colors,
        title=f'Ground-Truth Species on {feat_x} vs {feat_y}',
        labels={feat_x: feat_x, feat_y: feat_y}
        )
